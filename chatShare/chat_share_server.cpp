...
{
	if(argc<2){
		printf("usage: is ip_address portt_number \n",basename(argv[0]));
		return 1;
	}
	...
	while(!stop_server){
		...
		for(int i=0;i<number;i++){
			...
			if(sockfd == listenfd){
				...
				int confd = accept(listenfd,(struct sockaddr *)&client_address,&length);
				if(confd <0){
					printf("errno is %d\n",errno);
					continue;
				}
